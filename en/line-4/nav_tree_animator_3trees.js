class NavigationTreeAnimator {
    constructor() {
        ...
        this.tree3Buttons = Array.from(this.buttons).filter(btn => btn.dataset.tree === '3');
    ...
    getSavedState() {
        try {
            ...
            const activeNavButtons = localStorage.getItem('activeNavButtons2');
            if (activeNavButtons) {
            ...
            const savedCurrentButton = localStorage.getItem('treeAnimatorCurrentButton2');
    ...
    getTreeStartNumber(treeId) {
        // Повертаємо початковий номер для кожного дерева
        switch(treeId) {
            case '1': return 0;
            case '2': return 11;
            case '3': return 21; // Припускаємо, що третє дерево починається з 22
            default: return 0;
        }
    }
    ...
    animateTreeTransition(targetIndex, targetTree) {
        ...
        const treeStartNumber = this.getTreeStartNumber(targetTree);
